...
# ============================================================
# AI CLIENTS - Fixed: No global AI disable on Groq rate limit
# ============================================================
...
# FIXED: Scope rate limiting to Groq only
...
# ============================================================
# DATABASE - WITH WAL MODE AND CONNECTION POOLING
# ============================================================
...
# ============================================================
# CACHED SPY DATA - Fetch once per batch
# ============================================================
...
def get_spy_data():
    """Get SPY data with caching - fetched once per minute"""
...
# ============================================================
# ENHANCED NEWS SCRAPER - FIXED WITH CURRENTS API
# ============================================================

class EnhancedNewsScraper:
    def __init__(self):
        self.session = requests.Session()
        # Removed: self.session.verify = False - This was a security hole
        self.timeout = 6  # Increased from 3s for real network conditions
        self.user_agents = [
    ...
    def scrape_finviz(self, ticker):
        """Still server-rendered HTML, so this remains viable."""
        results = {'news': []}
    ...
    def scrape_yahoo_finance(self, ticker):
        """yfinance's news payload - handles both flat and nested formats"""
        results = {'news': []}
        try:
            stock = yf.Ticker(ticker)
            news = stock.news or []
            for item in news[:8]:
                # Handle both flat and nested content formats
    ...
    def scrape_google_news(self, ticker):
        """Google News RSS - works without JS"""
        results = {'news': []}
        try:
    ...
    def scrape_stocktwits(self, ticker):
        """Best-effort: StockTwits may 403, but try anyway"""
        results = {'news': []}
        try:
    ...
    def scrape_currents_api(self, ticker, company_name=''):
        """
        Replaces the 7 dead scrapers (marketwatch, tradingview,
        seekingalpha, bloomberg, cnbc, reuters, benzinga) with a real API.
        """
        results = {'news': []}
        ...
        try:
            ...
            if resp.status_code == 200:
                data = resp.json()
                for article in data.get('news', [])[:8]:
            ...
            logger.error(f"Currents API error for {ticker}: {e}")
        return results
    
    def fetch_all_news(self, ticker, company_name=''):
        """Fetch news from all working sources"""
        all_news = {}
        sources = {
            ...
            'currents': lambda t: self.scrape_currents_api(t, company_name),
        }
...
# ============================================================
# AI ANALYSIS ENGINE - FIXED: No global AI disable on Groq rate limit
# ============================================================
...
class AIAnalysisEngine:
    ...
    def get_ai_analysis(self, ticker, company, yahoo_data, sentiment_score, news_data, prediction_data):
        ...
        # 1. Try OpenAI (NOT disabled by Groq rate limit)
        if openai_client:
            try:
        ...
        # 4. Try Groq (only check rate limit here)
        if not result and groq_client and not is_groq_rate_limited():
            try:
...
# ============================================================
# NEWS SOURCES CONFIG - UPDATED TO MATCH REAL SOURCES
# ============================================================
...
NEWS_SOURCES = {
    ...
    "currents": {"name": "Currents API", "enabled": bool(CURRENTS_API_KEY), "icon": "🌐", "category": "aggregator"},
}
...
# ============================================================
# ENHANCED STOCK ANALYZER - WITH CACHED SPY AND FIXED NEWS
# ============================================================
...
class EnhancedStockAnalyzer:
    def __init__(self):
        ...
        # Per-request state instead of global
        self._request_state = {}
    ...
    def get_stock_data(self, ticker):
        ...
        try:
            ...
            if av_price and av_historical:
                ...
                adx = 25 + (random.random() - 0.5) * 20 if len(prices) >= 28 else 0
            ...
            # Use cached SPY data instead of downloading fresh each time
            relative_strength = 0
            try:
                spy = get_spy_data()
                if spy is not None and len(spy) >= 20 and len(hist) >= 20:
    ...
    def get_news_sentiment(self, ticker):
        company = ALL_STOCKS.get(ticker, {}).get('name', '')
        news_data = self.news_scraper.fetch_all_news(ticker, company)
...
# ============================================================
# MAIN ANALYSIS - WITH FIXED SCAN STATS (Thread-safe)
# ============================================================

# Thread-safe scan stats
...
# Per-request loaded tickers (not global)
...
def analyze():
    ...
    # Reset scan stats for this request
    with scan_stats_lock:
        scan_stats = {"technical": 0, "openai": 0, "claude": 0, "groq": 0, "gemini": 0, "total": 0}
    ...
    # Per-request loaded tickers (stored in session or request state)
    if 'loaded_tickers' not in session:
        session['loaded_tickers'] = []
    loaded_tickers = set(session['loaded_tickers'])
    ...
    if not tickers:
        if load_more:
            ...
            all_tickers = batch
        tickers = all_tickers
        session['loaded_tickers'] = list(loaded_tickers)
...
def status():
    return jsonify({
        ...
        'currents_api_available': CURRENTS_API_KEY is not None,
...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .ai-badge.gemini{background:rgba(66,133,244,0.2);color:#4285F4}
    ...
    <div class="sidebar" id="sidebar">
        ...
        <div style="margin-bottom:15px;padding:10px;background:rgba(102,126,234,0.1);border-radius:8px">
            ...
            <div style="font-size:11px;margin-top:4px">
                ...
                <span id="aiGemini" style="color:#4285F4">● Gemini</span>
            </div>
        </div>
...
function resetFilters() {
    ...
    selectedSources = ['finviz', 'yahoo', 'google_news', 'stocktwits'];
    ...
    try {
        ...
        if (res.success) {
            ...
            if (res.ai_availability) {
                ...
                document.getElementById('aiGemini').style.color = res.ai_availability.gemini ? '#4285F4' : '#666';
            }
...
function renderNewsFeed(container) {
    ...
    for (const [source, items] of Object.entries(groupedNews)) {
        const sourceIcon = {
            'Finviz': '📊', 'Yahoo Finance': '💹', 'Google News': '🔍',
            'StockTwits': '💬', 'Currents': '🌐'
        }[source] || '📰';
...
if __name__ == '__main__':
    print("\n" + "="*80)
    print("🚀 AI Stock Analyzer Pro - FIXED VERSION")
    print("="*80)
    ...
    print(f"🌐 Currents API: {'✅ Available' if CURRENTS_API_KEY else '❌ Not available'}")
    print("="*80)
    print("🔧 FIXES APPLIED:")
    print("   ✅ Replaced 7 dead scrapers with Currents API")
    print("   ✅ Fixed AI fallback chain (Groq rate limit no longer disables OpenAI)")
    print("   ✅ Thread-safe scan_stats with locking")
    print("   ✅ Per-request loaded_tickers stored in session")
    print("   ✅ Database WAL mode for better concurrency")
    print("   ✅ Cached SPY data (fetched once per minute)")
    print("   ✅ Removed SSL verification bypass (security fix)")
    print("   ✅ Added Gemini AI support")
    print("   ✅ Fixed Yahoo Finance news parsing")
    print("="*80)
